...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for index, record in enumerate(minibatch):
            record.parse_step(next_steps[index])
            if not record.buffer and len(record.stack) == 1:
                unfinished_parses.remove(record)
